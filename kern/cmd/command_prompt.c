...
bool isEqualNum(int num_of_args,int num_act_args){
	return num_of_args==num_act_args;
}



int handleEquelCommand(int num_of_args,int num_act_args,int i){
	if(isEqualNum(commands[i].num_of_args,num_act_args))
	            {

	                return i;
	            }
	            else if(commands[i].num_of_args==-1&&num_act_args>=1){
...
void handleUnEqual(int i,char*arguments){
	 char* command_name = commands[i].name;
	                            bool founded = 1;

	                            for (int j = 0; j < strlen(arguments); j++) {
	                                char* found_ptr = strchr(command_name, arguments[j]);

	                                if (found_ptr != '\0') {
	                                	command_name = found_ptr + 1;
	                                } else {
	                                	founded = 0;
	                                    break;
	                                }
...
LIST_INIT(&foundCommands);
int num_act_args=number_of_arguments-1;
    for (int i = 0; i < NUM_OF_COMMANDS; i++)
    {
        if (isEqualName(arguments[0],i))
        {
        return	handleEquelCommand(commands[i].num_of_args,num_act_args,i);


        }
        else {

        	handleUnEqual(i,arguments[0]);
